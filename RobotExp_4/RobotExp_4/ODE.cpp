...
double prev_err[MAX_JOINT_NUM] = { 0.0, 0.0 };
...
void PControl() {
	...
	double theta_curr[] = { 0, dJointGetHingeAngle(g_Joint[1]),
						dJointGetHingeAngle(g_Joint[2]) };   // Current Angle of each joint, [rad]
	double theta_ref[] = { 0, g_tar_q[0],
						g_tar_q[1] };                     // Reference Angle of each joint, [rad]
	double theta_ref_deg[] = { 0, g_tar_q[0] * RAD2DEG,
						   g_tar_q[1] * RAD2DEG };         // Reference Angle of each joint, [degree]
	double theta_err[MAX_JOINT_NUM] = { 0, };                  // Error of each joint angle
	double err_grad[MAX_JOINT_NUM] = { 0, };

	// PD Control term
	double p_term[MAX_JOINT_NUM] = { 0, };
	double d_term[MAX_JOINT_NUM] = { 0, };

	g_cur_q[0] = theta_curr[1];
	g_cur_q[1] = theta_curr[2];

	for (int i = 1; i < MAX_JOINT_NUM; i++) {
		// Exception handling
		while (theta_ref[i] * RAD2DEG > 180) {
			theta_ref[i] -= 360 * DEG2RAD;
		}
		while (theta_ref[i] * RAD2DEG < -180) {
			theta_ref[i] += 360 * DEG2RAD;
		}

		theta_err[i] = theta_ref[i] - theta_curr[i];            // error = reference - current
		if (theta_err[i] > 180 * DEG2RAD) {
			theta_err[i] -= 360 * DEG2RAD;
		}
		if (theta_err[i] < -180 * DEG2RAD) {
			theta_err[i] += 360 * DEG2RAD;
		}

		err_grad[i] = theta_err[i] - prev_err[i];               // delta error
		prev_err[i] = theta_err[i];                           // Update previous error
	}

	double Kp = 0.1, Kd = 0.01;                              // Set Kp, Kd gain
	double vel_ref[MAX_JOINT_NUM];                           // Velocity reference for control angle
	for (int i = 1; i < MAX_JOINT_NUM; i++) {
		p_term[i] = Kp * theta_err[i];                        // P term
		d_term[i] = Kd * err_grad[i] * dt;                     // D term
		vel_ref[i] = p_term[i] + d_term[i];                     // PD Contorller
