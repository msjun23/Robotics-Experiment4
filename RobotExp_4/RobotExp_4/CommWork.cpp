...
void CCommWork::_execute() {
	//_memname_tar = "Comm1Work_Controller_Target"
			...
			case 2:
				...
				if (checkSize == _packet.data.size) {
					if (check == _packet.data.check) {
						/*printf("pos : %f, velo : %f, cur : %f\n",
							_packet.data.pos / 1000 * RAD2DEG,
							_packet.data.velo / 1000 * RAD2DEG,
							_packet.data.cur / 1000 * 0.0683);*/
