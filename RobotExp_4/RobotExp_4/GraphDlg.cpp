...
BOOL CGraphDlg::OnInitDialog()
{
	...
	m_NTG_POS.SetRange(0, 25.0, -360.0, 360.0);
	m_NTG_POS.SetYGridNumber(8);
	...
	m_NTG_VEL.SetRange(0.0, 25.0, -50.0, 100.0);
	...
	m_NTG_TORQ.SetRange(0.0, 25.0, -0.5, 0.5);
...
void CGraphDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	if (m_dCnt >= 25.0) {
		m_NTG_POS.SetRange(m_dCnt - 25.0, m_dCnt, -360.0, 360.0);
		m_NTG_VEL.SetRange(m_dCnt - 25.0, m_dCnt, -50.0, 100.0);
		m_NTG_TORQ.SetRange(m_dCnt - 25.0, m_dCnt, -0.5, 0.5);
	}
	else {
		m_NTG_POS.SetRange(0, m_dCnt, -360.0, 360.0);
		m_NTG_VEL.SetRange(0, m_dCnt, -50.0, 100.0);
		m_NTG_TORQ.SetRange(0, m_dCnt, -0.5, 0.5);
	}

	// Visualize Position
	while (tar_motor_data.position * RAD2DEG > 180) {
		tar_motor_data.position -= 360 * DEG2RAD;
	}
	while (tar_motor_data.position * RAD2DEG < -180) {
		tar_motor_data.position += 360 * DEG2RAD;
	}
	m_NTG_POS.PlotXY(m_dCnt, tar_motor_data.position * RAD2DEG, 1);
	...
	/*if (curr_motor_data.position * RAD2DEG > 180) {
		curr_motor_data.position = 179.5 * DEG2RAD;
	}
	if (curr_motor_data.position * RAD2DEG < -180) {
		curr_motor_data.position = -179.5 * DEG2RAD;
	}*/
	m_NTG_POS.PlotXY(m_dCnt, curr_motor_data.position * RAD2DEG, 2);
	printf("Current Position: %f, Velocity: %f, Torque: %f\n",
		curr_motor_data.position * RAD2DEG, curr_motor_data.velocity * RAD2DEG, curr_motor_data.current);

	// Visualize Velocity
	m_NTG_VEL.PlotXY(m_dCnt, tar_motor_data.velocity * RAD2DEG, 1);
	m_NTG_VEL.PlotXY(m_dCnt, curr_motor_data.velocity * RAD2DEG, 2);

	// Visualize Current
	m_NTG_TORQ.PlotXY(m_dCnt, tar_motor_data.current * 0.0683, 1);      // Torque constant
	m_NTG_TORQ.PlotXY(m_dCnt, curr_motor_data.current * 0.0683, 2);      // Torque constant
